...
void CGame::UpdatePosPeople(char direction) {
	if (direction == 'w' && !(this->people.GetTopLeft().GetY() == this->topleft.GetY())) {
		this->people.Move(0, 1);
	}
	else if (direction == 's' && !(this->people.GetBotRight().GetY() == this->botright.GetY())) {
		this->people.Move(0, -1);
	}
	else if (direction == 'a' && !(this->people.GetTopLeft().GetX() == this->topleft.GetX())) {
		this->people.Move(-1, 0);
	}
	else if (direction == 'd' && !(this->people.GetBotRight().GetX() == this->botright.GetX())) {
		this->people.Move(1, 0);
	}
}
